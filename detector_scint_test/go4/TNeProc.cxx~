...
// #endif // NO_ANNUAL
		...
		d1Vt[i]   = MakeTH1('I', Form("Vt/Vts_%2.2d",i)
				, Form("Veto time CH %2.2d",i), 16384, 0., 16384.);
		...
		d1Rxt[i]   = MakeTH1('I', Form("Rxt/Rxts_%2.2d",i)
				, Form("R x time CH %2.2d",i), 16384, 0., 16834.);
		...
		d1Ryt[i]   = MakeTH1('I', Form("Ryt/Ryts_%2.2d",i)
				, Form("R y time CH %2.2d",i), 16384, 0., 16384.);
		...
		d1R20t[i]   = MakeTH1('I', Form("R20t/R20ts_%2.2d",i)
				, Form("R 20 time CH %2.2d",i), 16384, 0., 16384.);
		d1R20c[i]   = MakeTH1('D', Form("R20c/R20cs_%2.2d",i)
				, Form("R 20 energy MeV %2.2d",i), 4000, -2., 98.);

		d1DSSD_Xa[i]   = MakeTH1('I', Form("L_DSSD_Xa/L_DSSD_Xas_%2.2d",i)			
				, Form("L DSSD_X amp CH %2.2d",i), 8192, 0., 8192.);
		d1DSSD_Xt[i]   = MakeTH1('I', Form("L_DSSD_Xt/L_DSSD_Xts_%2.2d",i)
				, Form("L DSSD_X time CH %2.2d",i), 16384, 0., 16384.);
		d1DSSD_Xc[i]   = MakeTH1('D', Form("L_DSSD_Xc/L_DSSD_Xcs_%2.2d",i)
				, Form("L DSSD_X energy MeV %2.2d",i), 500, -2., 98.);

		d1DSSD_Ya[i]   = MakeTH1('I', Form("L_DSSD_Ya/L_DSSD_Yas_%2.2d",i)			
				, Form("L DSSD_Y amp CH %2.2d",i), 8192, 0., 8192.);
		d1DSSD_Yt[i]   = MakeTH1('I', Form("L_DSSD_Yt/L_DSSD_Yts_%2.2d",i)
				, Form("L DSSD_Y time CH %2.2d",i), 16384, 0., 16384.);
		d1DSSD_Yc[i]   = MakeTH1('D', Form("L_DSSD_Yc/L_DSSD_Ycs_%2.2d",i)
				, Form("L DSSD_Y energy MeV %2.2d",i), 500, -2., 98.);
	
	}
	d1DSSD_Xmult = MakeTH1('I', "L_DSSD_Xmult",			
				 "L DSSD_X mult (by time) ", 32, 0., 15.);
	d1DSSD_Xn    = MakeTH1('I', "L_DSSD_Xn",			
				 "L DSSD_X hit position (by time) ", 16, 0., 15.);
	d1DSSD_Xdep  = MakeTH1('D', "L_DSSD_Xdep",			
			       "L DSSD_X deposit for mult==1 ", 500, -2., 98.);
	d1DSSD_Ymult   = MakeTH1('I', "L_DSSD_Ymult",			
				 "L DSSD_X mult (by time) ", 32, 0., 15.);
	d1DSSD_Yn   =   MakeTH1('I', "L_DSSD_Yn",			
				"L DSSD_Y hit position (by time) ", 16, 0., 15.);
	d1DSSD_Ydep   = MakeTH1('D', "L_DSSD_Ydep",			
				"L DSSD_Y deposit for mult==1 ", 500, -2., 98.);

  	// left CsI
	d2LCsiCount=MakeTH2('C',"LCsI2D/LCsiCount","CsI wall Profile",
			    4,0.,3.,4.,0.,3.,"X","Y");
	for ( int i = 0; i < 16; ++i) {
		d2LCsISi[i] = MakeTH2('I', Form("LCsI2D/d2LCsISi_%2.2d", i),
				      Form("Left CsI amplitude --- dE ch%2.2d", i),
				      1024, 0., 8191.,256, 0., 100.,"E","dE");
		d1LCsIa[i] = MakeTH1('I', Form("LCsIa/LCsIa_%2.2d", i),
				     Form("Left CsI amplitude ch%2.2d", i), 8192, 0., 8192.);
		d1LCsIt[i] = MakeTH1('I', Form("LCsIt/LCsIt_%2.2d", i),
				     Form("Left CsI time ch%2.2d", i), 16384, 0., 16384.);
		d1LCsIc[i] = MakeTH1('D', Form("LCsIc/LCsIc_%2.2d", i),
				     Form("Left CsI energy ch%2.2d", i), 8192, 0., 300.);
	...
	*/
	d2edeR20  = MakeTH2('D', "ede R20", "ede R20", 300,0.,50.,120,0.,6.,"eR1mm","deR20");
...
//
	...
	d1tF6[0]  = MakeTH1('I',"F6/TDC/tF6_TDC_L","F6 TDC  left", 15000, 0, 14999);
	d1tF6[1]  = MakeTH1('I',"F6/TDC/tF6_TDC_R","F6 TDC right", 15000, 0, 14999);
	d1tF6[2]  = MakeTH1('I',"F6/TDC/tF6_TDC_U","F6 TDC    up", 15000, 0, 14999);
	d1tF6[3]  = MakeTH1('I',"F6/TDC/tF6_TDC_D","F6 TDC  down", 15000, 0, 14999);
//
...
//-----ToF-----
	...
	d1aF6 = MakeTH1('D', "ToF/aF6", "amplitude of plastic F6", 4000,0.,4000.,"amp F6");
	d1ToF6 = MakeTH1('D', "ToF/ToF6", "time of flight", 14000,-100.,600.,"ToF(ns)");
	d2ToFaF6 = MakeTH2('D', "ToF/ToFaF6", "ToF vs aF6", 550,50.,600.,400,0.,4000.,"ToF","aF6");
			...
			case 100: //VME_0, see setup.usf
			  ...
			  if (m_bool) {//in case of MESYTEC data
						...
						case 0: //MESYTEC data
							...
							switch (module_id) {
							case  1:	// mTDC: time R20  & Rveto
								chnl= (idata[n] >> 16)&0x3f;
								adc = idata[n] & 0xffff; //only 12 bits
								if(chnl<16) {
									NeEvent->R20t[chnl] = adc;
//									NeEvent->St[chnl] = adc;
								} else if(chnl<32) {
									NeEvent->Vt[chnl-16] = adc;
								}
								break;

							case  2:	// mADC: amp R20  & Rveto
								chnl= (idata[n] >> 16)&0x1f;
								adc = idata[n] & 0x1fff; //only 13 bits
								if(chnl<16) {
...
//									NeEvent->Sa[chnl] = adc;
									...
									NeEvent->Va[chnl-16] = adc;
								}
								break;

							case  3:	// mTDC: L 1500 DSD 
								chnl= (idata[n] >> 16)&0x3f;
								adc = idata[n] & 0xffff; 
								if(chnl<16) {
									NeEvent->timeDSSD_L_X[chnl] = adc;
									//NeEvent->Vt[chnl] = adc;
								} 
								else if(chnl<32) {
									NeEvent->timeDSSD_L_Y[chnl-16] = adc;
									//NeEvent->R20t[chnl-16] = adc;
								} 
								break;

							case  4:	// mADC: L 1500 DSD
								chnl= (idata[n] >> 16)&0x1f;
								adc = idata[n] & 0x1fff; //only 13 bits
								if(chnl<16) {
									NeEvent->ampDSSD_L_X[chnl] = adc;
									//NeEvent->Va[chnl] = adc;
									}
								else if(chnl<32) {
									NeEvent->ampDSSD_L_Y[chnl-16] = adc;
							...
							case 13:	//mTDC: ToF
								...
								else if(chnl<12) NeEvent->tF6[chnl-4] = adc;
							...
							case 14:	//mQDC:
								...
								else if(chnl<12) NeEvent->F6[chnl-4] = adc;
					...
					if (m_bool) //in case of MESYTEC data
					{
						...
						switch(typ)
						{
							...
							case 0: //MESYTEC data
								...
								switch (module_id)
								{
									case 3:	// mTDC: CsI time 
										chnl= (idata[n] >> 16)&0x3f;
										adc = idata[n] & 0xffff; //only 16 bits
										if(chnl<16) {
											NeEvent->tCsI[chnl] = adc; 
										}
										break;
...
//
	...
	if(NeEvent->F6[0]>0&&NeEvent->F6[1]>0&&NeEvent->F6[2]>0&&NeEvent->F6[3]>0)
	  af6 = (NeEvent->F6[0]+NeEvent->F6[1]+NeEvent->F6[2]+NeEvent->F6[3])/4. + gRandom->Uniform();
	...
	if(NeEvent->tF6[0]>0&&NeEvent->tF6[1]>0&&NeEvent->tF6[2]>0&&NeEvent->tF6[3]>0)
	  tf6 = (NeEvent->tF6[0]+NeEvent->tF6[1]+NeEvent->tF6[2]+NeEvent->tF6[3])/4. + gRandom->Uniform();
...
// #endif // NO_ANNUAL
		...
		//if(NeEvent->timeDSSD_L_X[n]>0)
		{
			d1DSSD_Xa[n]->Fill(NeEvent->ampDSSD_L_X[n]);
			d1DSSD_Xt[n]->Fill(NeEvent->timeDSSD_L_X[n]);
		}
		//if(NeEvent->timeDSSD_L_Y[n]>0)
		{
			d1DSSD_Ya[n]->Fill(NeEvent->ampDSSD_L_Y[n]);
			d1DSSD_Yt[n]->Fill(NeEvent->timeDSSD_L_Y[n]);
		}

		{
			d1LCsIa[n]->Fill(NeEvent->aCsI[n]);
			d1LCsIt[n]->Fill(NeEvent->tCsI[n]);
		}
...
//______ calibrated spectra __________________________
	NeEvent->multDSSD_L_X=0;
	NeEvent->multDSSD_L_Y=0;

	for (int n=0; n<16; n++) {
		
		if(NeEvent->timeDSSD_L_X[n]>0&&NeEvent->ampDSSD_L_X[n]<7000){
			value1 =
				double(NeEvent->ampDSSD_L_X[n])+gRandom->Uniform();
			value2 = ((TNeAnalysis*)go4)->p_Xa->Energy(value1, n);
			NeEvent->depDSSD_L_X[n] = value2;
			d1DSSD_Xc[n]->Fill(value2);
			NeEvent->num_L_X[NeEvent->multDSSD_L_X]=n;
			NeEvent->dep_L_X[NeEvent->multDSSD_L_X] = value2;
			NeEvent->multDSSD_L_X++;
		}
		if(NeEvent->timeDSSD_L_Y[n]>0&&NeEvent->ampDSSD_L_Y[n]<7000){
			value1 =
				double(NeEvent->ampDSSD_L_Y[n])+gRandom->Uniform();
			value2 = ((TNeAnalysis*)go4)->p_Ya->Energy(value1, n);
			NeEvent->depDSSD_L_Y[n] = value2;
			d1DSSD_Xc[n]->Fill(value2);
			NeEvent->num_L_Y[NeEvent->multDSSD_L_Y]=n;
			NeEvent->dep_L_Y[NeEvent->multDSSD_L_Y] = value2;
			NeEvent->multDSSD_L_Y++;
		}
		/*	if(NeEvent->Yt[n]>0){
	...
	d1DSSD_Xmult->Fill((NeEvent->multDSSD_L_X));
	if(NeEvent->multDSSD_L_X==1){
		d1DSSD_Xn->Fill((NeEvent->num_L_X[0]));
		d1DSSD_Xdep->Fill((NeEvent->dep_L_X[0]));
	}
	d1DSSD_Ymult->Fill((NeEvent->multDSSD_L_Y));
	if(NeEvent->multDSSD_L_Y==1){
		d1DSSD_Yn->Fill((NeEvent->num_L_Y[0]));
		d1DSSD_Ydep->Fill((NeEvent->dep_L_Y[0]));
	}
...
// #endif // NO_ANNUAL
		if(NeEvent->Vt[n]>0){
			...
			value2 = dynamic_cast < TNeAnalysis* > (go4)->p_Va->Energy(value1, n);
			d1Vc[n]->Fill(value2);
			NeEvent->Vc[n] = value2;
			mVa++;
			eVa = value2;
	...
	// CsI Raw 2D histogram
	if(NeEvent->multDSSD_L_X==1){
		for (int n=0; n<16; n++) {
			d2LCsISi[n]->Fill(NeEvent->aCsI[n],NeEvent->dep_L_X[0]);	
		}
	}
/*
