...
#ifdef CAEN
	static struct v1725_reg *v1725_ptr[NUMBER_OF_CARDS]; //digitizer
	static struct v1725_EF_reg *v1725_EF_ptr[NUMBER_OF_CARDS]; //digitizer
	static struct v1725_ch_reg *v1725_ch_ptr_6[NUMBER_OF_CHANNELS]; //digitizer
	//static struct v1725_ch_reg *v1725_ch_ptr_7[NUMBER_OF_CHANNELS]; //digitizer

	static FILE *CAENfile;
	static FILE *CAEN_logfile;
	char file_name[] = "dg01.dat";
	char log_file_name[] = "caen.txt";

	#include "./caen/v1725_fct.c"
#endif
...
int f_user_get_virt_ptr (long  *pl_loc_hwacc, long  pl_rem_cam[])
{
	...
	pl_trig_vme_cvt = (long*) ( (long) pl_loc_hwacc + 0xc );
		...
		for(i=0;i<NUMBER_OF_CARDS;i++){
			...
			#ifdef CAEN
				v1725_ptr[i] = (struct v1725_reg *)((long) caen_base+ (long)((i)*CARDS_OFF) + (long)(V1725_OFF));
				v1725_EF_ptr[i] = (struct v1725_EF_reg *)((long) caen_base+ (long)((i)*CARDS_OFF) + (long)(V1725_EF_OFF));
			#endif
//
			#ifdef MESYTEC  
...
//
		...
		{
			v1725_ch_ptr_6[i] = (struct v1725_ch_reg *)((long) caen_base+ (long)((6)*CARDS_OFF) + (long)(V1725_CH_OFF) + (long)((i)*V1725_CH_DIFF));
			//v1725_ch_ptr_7[i] = (struct v1725_ch_reg *)((long) caen_base+ (long)((7)*CARDS_OFF) + (long)(V1725_CH_OFF) + (long)((i)*V1725_CH_DIFF));
		}
			#endif
	...
	switch (bh_crate_nr)
		{
			case 0:             /* crate 0 */
				...
				#ifdef MESYTEC            
					card=3;
					#include "./mesytec/init_mtdc32.c"		// CsI(Tl) 16 time
					mtdc32_ptr[card]->tdc_resolution = 9;		// 9 - 500 ps; 7 -> 125 ps
					mtdc32_ptr[card]->bank0_win_start = 15884;	// 16384 -> trigger
					mtdc32_ptr[card]->bank0_win_width = 2000;	// in ns
					mtdc32_ptr[card]->trig_select = 0;		// trigger from ECL

					card=4;
					#include "./mesytec/init_madc32.c"		// CsI(Tl) 16 amp
  					usleep(10000);
